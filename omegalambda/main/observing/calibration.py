...
class Calibration(Hardware):
    ...
    def take_flats(self, ticket):
        ...
        flats_path = os.path.join(self.image_directories[ticket], 'Flats_{}'.format(ticket.name))
            ...
            for f in filters:
                ...
                while j < self.config_dict.calibration_num:
                    ...
                    self.camera.onThread(
                        self.camera.expose, self.filter_exp_times[f], self.filterwheel_dict[f], save_path=os.path.join(flats_path, image_name), type="light"
                    )
                    self.camera.image_done.wait()
                    median = filereader_utils.mediancounts(os.path.join(flats_path, image_name))
                    if scaled is False and median < self.config_dict.saturation:
                        ...
                        j += 1
            files = os.listdir(flats_path)
            for file in files:
                file = os.path.join(flats_path, file)
    ...
    def take_darks(self, ticket):
        ...
        darks_path = os.path.join(self.image_directories[ticket], 'Darks_{}'.format(ticket.name))
        if not os.path.exists(darks_path):
            os.mkdir(darks_path)
        else:
        ...
        if ticket.camera and ticket.camera == "NIR":
            ...
            for f in filters:
                for j in range(self.config_dict.calibration_num):
                    ...
                    match = False
                    for name in os.listdir(darks_path):
                        if name == image_name:
                            match = True
                    if match:
                        continue
